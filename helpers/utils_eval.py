...
################### MODIFIED - 08/01/2019
'''
    These metrics are adjusted to censoring;
'''
...
def weighted_c_index(T_train, Y_train, Prediction, T_test, Y_test, Time):
    ...
    for i in range(N):
        ...
        Q[i, np.where(Prediction[i] == Prediction)] = 0.5 # give weights
...
def weighted_brier_score(T_train, Y_train, Prediction, T_test, Y_test, Time):
    ...
    return np.mean(W*(y_true - Prediction)**2)
